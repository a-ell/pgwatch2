...
are monitored with 60s interval) without breaking a sweat (<20% load). When a single node where the metrics gatherer/collector/daemon
is running is becoming a bottleneck, one can also do "sharding", limiting monitored databases for that node based on the Group label
(--group), which is just a string for logical grouping.
